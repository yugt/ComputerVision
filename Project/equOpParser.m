...
for i=1:max(max(label))
    [row, col]=find(label==i);
    crop=selector(label,i);

    if (abs((max(row)-min(row)+1)/(max(col)-min(col)+1)-1)<0.1) ...
            && nnz(crop)/(size(crop,1)*size(crop,2))<0.4
%         imshow(label(min(row):max(row),min(col):max(col)));
        ...
        s=[nnz(im0~=im1) nnz(im0~=im2) nnz(im0~=im3) nnz(im0~=im4)...
            ...
            0 nnz(im1~=im2) nnz(im1~=im3) nnz(im1~=im4)...
            nnz(im1~=im5) nnz(im1~=im6) nnz(im1~=im7);...
            0 0 nnz(im2~=im3) nnz(im2~=im4)...
            nnz(im2~=im5) nnz(im2~=im6) nnz(im2~=im7);...
            0 0 0 nnz(im3~=im4) nnz(im3~=im5) nnz(im3~=im6) nnz(im3~=im7);...
            0 0 0 0 nnz(im4~=im5) nnz(im4~=im6) nnz(im4~=im7);...
            0 0 0 0 0 nnz(im5~=im6) nnz(im5~=im7);...
            0 0 0 0 0 0 nnz(im6~=im7)];
        x(i)=sum(sum(s));
        if sum(sum(s))<4096
            [row,col]=find(selector(label,i));
            hr=hist(row);
            hc=hist(col);
            if min(hr)/max(hr)<0.25 && min(hc)/max(hc)<0.25
